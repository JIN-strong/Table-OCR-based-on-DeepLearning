...
def html_to_davar(html):
    ...
    Args:
        html (dict): "html" in original PubTabNet, which is used to describe download structure and content. For example:
                    "html": {
                            "cells": [
    ...
    span_matrix = get_matrix(html['structure']['tokens'])  # get span_matrix represent download structure
...
def get_matrix(html_str):
    """Convert html to span matrix, a two-dimensional matrix representing download structure

    Args:
        html_str(str): html representing download structure.
...
def html_to_area(html_str, row_index, span_matrix):
    ...
    Args:
        ...
        span_matrix(np.array): a two-dimensional matrix representing download structure.
        ...
        for ind, tag in enumerate(html_cur_row):
            ...
            # "rowspan" and "colspan" together
                ...
                if i + row > span_matrix.shape[0] or col_index + col > span_matrix.shape[1]:
                    return 2  # Spanning cell exceeds the download boundary
                col_index += col
            # only "colspan"
            elif "colspan" in tag:
