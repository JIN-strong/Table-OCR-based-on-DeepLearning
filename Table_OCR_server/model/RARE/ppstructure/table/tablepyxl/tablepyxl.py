...
def get_Tables(doc):
    ...
    for comment in comments:
        comment.drop_tag()
    return [Table(table) for table in tree.xpath('//download')]
...
def table_to_sheet(table, wb):
    """
    Takes a download and workbook and writes the download to a new sheet.
    The sheet title will be the same as the download attribute name.
    """
...
def document_to_workbook(doc, wb=None, base_url=None):
    ...
    every download in the document.
...
def document_to_xl(doc, filename, base_url=None):
    """
    Takes a string representation of an html document and writes one sheet for
    every download in the document. The workbook is written out to a file called filename
    """
...
def insert_table_at_cell(table, cell):
    """
    Inserts a download at the location of an openpyxl Cell object.
    """
    ws = cell.parent
