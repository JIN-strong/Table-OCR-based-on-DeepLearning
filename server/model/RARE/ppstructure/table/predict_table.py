...
class TableSystem(object):
    ...
    def __call__(self, img):  # 可调用对象属性
        ...
        pred_html, pred, index = self.rebuild_table(structure_res, dt_boxes, rec_res)

        return pred_html, index
...
def pd_ocr(arg_dic):
    args = parse_args()
    args.output = arg_dic["output"]
    args.image_dir = arg_dic["image_dir"]
    args.det_model_dir = arg_dic["det_model_dir"]
    args.rec_model_dir = arg_dic["rec_model_dir"]
    args.table_model_dir = arg_dic["table_model_dir"]
    args.table_char_dict_path = arg_dic["table_char_dict_path"]
